...
PHP_MINFO_FUNCTION(sm) {
    ...
    php_info_print_table_row(2, "Version", PHP_SM_VERSION);
    php_info_print_table_row(2, "Author", "Ilya Sabelnikov <fruit.dev@gmail.com>");
    php_info_print_table_end();
}
/* }}} */

/* {{{ proto string sm_version()
   Return the php SM version */
PHP_FUNCTION(sm_version) {
    if (return_value_used) {
        RETURN_STRING(PHP_SM_VERSION, strlen(PHP_SM_VERSION));
    } else {
        php_printf("PHP SM Version: %s\n", PHP_SM_VERSION);
    }
}

/* }}} */

static void sm_letter_pairs(const char *str, uint len, zval *return_value, int *pairs_count) /* {{{ */ {
    ...
    *pairs_count += pairs_size;
    ...
    for (i = 0; i < pairs_size; i++) {
        ...
        if (SUCCESS == zend_hash_find(Z_ARRVAL_P(return_value), (const char *) ret->val, ret->len, (void **) &tmp)) {
            Z_LVAL_PP(tmp)++;
        } else {
            add_assoc_long_ex(return_value, (const char *) ret->val, ret->len, 1);
        }
...
static void sm_word_letter_pairs_exp(mbfl_string str, zval *return_value, int *pairs_count) /* {{{ */ {
    ...
    for (zend_hash_internal_pointer_reset_ex(Z_ARRVAL_P(pairs_in_word), &pos);
        ...
        if (0 < Z_STRLEN_PP(data)) {
            sm_letter_pairs(Z_STRVAL_PP(data), (uint) Z_STRLEN_PP(data), return_value, pairs_count);
        }
    }
...
PHP_SM_API double sm_strike_match(const char *str_a_val, uint str_a_len, const char *str_b_val, uint str_b_len) /* {{{ */ {
    zval *word_a_pairs;
    zval *word_b_pairs;
    ...
    if (0 == (str_a.len + str_b.len) || 0 == strcmp((const char *) str_a.val, (const char *) str_b.val)) {
        return 1.0;
    }
...
PHP_FUNCTION(strike_match) {
    ...
    if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "ss", &str_a_val, &str_a_len, &str_b_val, &str_b_len) == FAILURE) {
        return;
    }
